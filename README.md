...
The incoming USB serial data is first copied into a buffer inside the USB interrupt callback. The buffer is then checked using the main while loop and compared against a list of known commands. The list of commands are stored as strings to compare against (ignoring case) the incoming command. 

If a match is found, the command is processed and a response is generated via USB CDC using the `CDC_Transmit_FS` function. If a command match is not found, the response will be `"Invalid Commmand! Type 'help'"`.

> **Note:** This project must receive a full command packet inside of one USB interrupt service routine, otherwise the command buffer will be reset. To prevent this, a command index can be used to append data to the command buffer. The command can then be processed and reset after a newline character (or other command termination symbol) is received or the buffer is filled.
...
* **`main()`**: Initializes system clocks, peripherals, and resets data buffers.
  * *Infinite loop*: Calls `readCommand`, blinks LED, and uses a blocking delay.
